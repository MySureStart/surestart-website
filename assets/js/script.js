...
(function() {
  ...
  function initRotatingQuotes() {
    ...
    rotatingQuotesContainers.forEach(container => {
      ...
      function updateContainerHeight(index) {
        ...
        // On mobile (< 768px), CSS handles height naturally via position: relative
        // on the active item, so skip JS height calculation
        if (window.innerWidth < 768) {
          quoteContent.style.height = '';
          quoteContent.style.minHeight = '';
          return;
        }
        ...
        if (activeItem) {
          ...
          // Set exact measured height on desktop only
          quoteContent.style.height = height + 'px';
        }
      }
      
      // Add resize handler to recalculate on orientation changes
      let rotatingQuotesResizeTimeout;
      window.addEventListener('resize', () => {
        clearTimeout(rotatingQuotesResizeTimeout);
        rotatingQuotesResizeTimeout = setTimeout(() => {
  ...
  function initPopupModals() {
    ...
    function openPopup(overlay) {
      ...
      const closeBtn = overlay.querySelector('.notify-popup-close');
      if (closeBtn) {
    ...
    waitlistTriggers.forEach(trigger => {
      trigger.addEventListener('click', (e) => {
        e.preventDefault();
        openPopup(waitlistOverlay);
      });
    });
    
    // Set up notify popup triggers
    notifyTriggers.forEach(trigger => {
      trigger.addEventListener('click', (e) => {
        e.preventDefault();
        openPopup(notifyOverlay);
      });
    });
